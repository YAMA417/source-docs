...
**書くのはインターフェースと構造まで。** API なら呼び出す側から見えるもの
（リクエスト・レスポンス・ステータスコード）、DB ならテーブルの構造、画面なら遷移。
実装の挙動（処理の流れ・トランザクション・トリガや RLS の中身）は書かない。
...
各資料の章の定義は `skills/source-docs/templates/` にある。
書き方の共通規則は `skills/source-docs/references/writing.md`。
